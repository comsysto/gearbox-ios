...
struct ExploreView: View {
  ...
  // MARK: - FUNCTIONS
  private func searchBlogsAndUsers(_ term: String) {
    if (term.isEmpty) {
      viewModel.resetState()
      return
    }
    
    viewModel.searchBlogsOrUsers(term)
  }
}
...
private extension ExploreView {
  ...
  func renderSearchInput() -> some View {
    VStack {
      GearboxTextField("Search...", text: $viewModel.state.searchTerm, type: .text)
        .onChange(of: viewModel.state.searchTerm) { _, term in
          searchBlogsAndUsers(term)
        }
      ...
      case .result:
        ScrollView {
          ForEach(0..<viewModel.state.authorResultList.count, id: \.self) { index in
            renderProfileInfo(for: viewModel.state.authorResultList[index])
          }
        } //: SCROLL VIEW
      case .noResult:
        VStack(alignment: .center) {
  ...
  @ViewBuilder
  func renderProfileInfo(for user: Author) -> some View {
    HStack {
